...
node {
    ...
    stage('Obtain and cleandata') {
        ...
        sh 'java -jar loadparkingdata/cleanparkingdata.jar
    ...
    stage('Convert CSV to RDF') {
         ...
         sh 'java -jar csv2rdf/csvparkingsclean2rdf.jar
              donostiaparkingsclean.csv
              donostiaparkingsclean.nquads
              http://lod.eurohelp.es/dataset/parkings'
    }
    stage('Upload RDF to blazegraph') {
         ...
         sh 'curl -X POST
              ...
              --data-binary
              @donostiaparkingsclean.nquads
              http://172.16.0.81:58080/blazegraph/sparql'
    }
}
