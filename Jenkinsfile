...
node {
    try {
        ...
        stage('Convert CSV to RDF') {
            def ret = sh(script: 'java -jar CSVToRDFParkings/parkingsrdfcreator.jar ' + CSVParkings + ' ' + NewCSVParkings + ' ' + RmlConfigurationFile + ' ' + RDFParkings+ ' ' + entorno, returnStdout: true)
        ...
        stage('Notify failure') {
            ...
            mail(to: 'dmuv7@hotmail.com',
                ...
                body: "Ha fallado la ejecuci�n de '${env.JOB_NAME}', el error se ha dado en: " + date + ". Revisa el error en http://"+entorno+"/jenkins/job/Replicate-Donosti-Parkings/${env.BUILD_NUMBER}",
                mimeType: 'text/html');
